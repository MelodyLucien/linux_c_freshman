...
//main() ---------> inotify dev_path(/dev/input/) -------> scan()----->while(1) poll------

//   getopt         monitor filesystem           collect fds device      read content from device fds
...
//get the label name of the exact value
...
static void print_hid_descriptor(int bus, int vendor, int product)
{
    ...
    while((de = readdir(dir))) {
        //he strstr() function finds the first occurrence of the substring needle in the string haystack.  The terminating null bytes ('\0') are not compared.
